...
*/
define('simple_feed_list_version','2.1');
add_shortcode('feedlist','simple_feed_list_sc');

// Shortcode to return a feed list
function simple_feed_list_sc($paras="",$content="") {
    extract(shortcode_atts(array('listurl'=>'','limit'=>'','desc'=>'','more'=>'','target'=>'','nofollow'=>''),$paras));
    return "<ul>\n".get_feed_code($listurl,$limit,$desc,$more,$target,$nofollow)."</ul>\n";
}
...
function simple_feed_list($feed_url,$add_paras,$desc_flag="") {
    ...
    echo get_feed_code($feed_url,$list_limit,$desc_flag,$more,$target,$nofollow);
    return;
}

// Produce code for feed list
function get_feed_code($feed_url,$list_limit="20",$desc_flag="",$more="",$target="_blank",$nofollow="") {    
    ...
    if ($check_failure!==true) {
        ...
        if ($array=="") {
            ...
            if (($list_link!="")&&($more!="no")) {$code.="<li><a href=\"".$list_link."\" target=\"".$target."\"".$nofollow.">More...</a></li>\n";}
        }
    }
